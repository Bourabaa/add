...
from matcher import combine_request_text, normalize_text
...
DEFAULT_REPORT_PATH = PROJECT_PATHS.embedding_report_path
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate the ONNX multilingual embeddings matcher on strong-labeled feedback rows."
    )
    parser.add_argument(
        "--organizations",
        type=Path,
...
def main() -> int:
    ...
    for row in labeled_rows:
        ...
        predicted_top2 = results[1] if len(results) > 1 else {}
        predicted_slugs = [result["organization_slug"] for result in results]
        routing_decision = classify_route_decision(
            results=results,
        ...
        if routing_decision["decision"] == "auto_route":
            auto_route_total += 1
            auto_route_correct += int(is_top1_correct)
        else:
        ...
        report_rows.append(
            {
                ...
                "predicted_top2_slug": predicted_top2.get("organization_slug", ""),
                "predicted_top2_name": predicted_top2.get("organization", ""),
                "predicted_top2_confidence": predicted_top2.get("confidence", ""),
                "top1_margin": routing_decision.get("margin", ""),
                "routing_decision": routing_decision.get("decision", ""),
                "routing_reasons": "|".join(routing_decision.get("reasons", [])),
    ...
    auto_route_precision = ((auto_route_correct / auto_route_total) * 100) if auto_route_total else 0.0
    auto_route_coverage = ((auto_route_total / total) * 100) if total else 0.0
    ...
    print(
        f"Auto-route rows: {auto_route_total}/{total} = {auto_route_coverage:.2f}% coverage "
        f"| precision: {auto_route_correct}/{auto_route_total or 1} = {auto_route_precision:.2f}%"
    )
    print(f"Manual-review rows: {manual_review_total}/{total}")
